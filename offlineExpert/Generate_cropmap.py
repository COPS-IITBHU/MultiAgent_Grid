...
parser.add_argument('--map_width', type=int, default=10) #map size
parser.add_argument('--map_density', type=float, default=0.1) #number of obstacles per unit area
parser.add_argument('--map_complexity', type=float, default=0.01) #difficulty of the map - for expert solver
parser.add_argument('--num_agents', type=int, default=4) #num of robots
parser.add_argument('--num_dataset', type=int, default=1) #number of cases
parser.add_argument('--random_map', action='store_true', default=False) #need to check
parser.add_argument('--gen_CasePool', action='store_true', default=False) #not used in this script
parser.add_argument('--chosen_solver', type=str, default='ECBS') #type of expert solver
parser.add_argument('--num_caseSetup_pEnv', type=int, default=100) #not used in this script
...
parser.add_argument('--gen_map_type', type=str, default='random')
...
class EnvsGen:
    ...
    def mapGen(self, width=10, height=10, complexity=0.01, density=0.1,
        ...
        if self.gen_map_type == 'maze':
            ...
            complexity = int(complexity * (5 * (world_size[0] + world_size[1]))) #by default 0.01 * 5 * (20) = 1
            # size of components
            density = int(density * ((world_size[0] // 2) * (world_size[1] // 2))) #default = 2.5 = 2
            ...
            for i in range(density):
                ...
                maze[y, x] = 1
                #this loop is used to add obstacles, it takes the initial x, y then randomly at distance 2 from that adds another obstacle, and in the same
                #direction adds an obstacle at a distance 1
                #basically --- or 3 block wall, and then randomly select where to go next
                #to create maze
            ...
            if path_size == 0: # no regulatoin on path size
                #Keep all borders free of obstacles
                maze[0] = 0
                maze[width-1] = 0
                maze[:, 0] = 0
                maze[:, height-1] = 0
                ...
                #iteratively remove obstacles (full row/column) to get required density
                ...
                #take path size required for bot to move
            ...
            #make several small mazes in the world
            maze = np.zeros(world_size, dtype=np.int64)
    ...
    def genMap(self):
        ...
        """
        #number of test cases = num_data = data_points
